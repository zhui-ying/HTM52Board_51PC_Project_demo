...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			plotXAxis1->ScaleRange->Span = 0.000694444446708076;
...
private: System::Void timer100ms_Tick(System::Object^  sender, System::EventArgs^  e) {
			 //读取公共区，刷新显示值
			double Hot_u = ((double)(modbus->modbus_Addt.AD_Hot)/255)*(edit_ref->Value->AsDouble);
			Hot_u = Math::Round(Hot_u,2);//取两位小数
			//限幅
			if(Hot_u > edit_ref->Value->AsDouble) Hot_u = edit_ref->Value->AsDouble;
			if(Hot_u < 0) Hot_u = 0;
			ang_u->Value = Hot_u;
			edit_u->Value = Hot_u;
			plot_U->Channels[0]->AddXY(System::DateTime::Now,Hot_u);//绘图
			 //发送读取所有值指令
			 //刷新数据
